...
const ExamTaking = () => {
    ...
    // Popup State
    const [popupState, setPopupState] = useState({ isOpen: false, type: 'info', title: '', message: '', onAction: null });
        ...
        const fetchInitialState = async () => {
            try {
                ...
                if (!attemptRes.data.data) {
                    ...
                    const durationSecs = (examData.durationInMinutes || 60) * 60;
                    setTimeRemaining(durationSecs);
                }
            } catch (error) {
                console.error("Failed to fetch exam data", error);
                showPopup('error', 'Lỗi tải đề thi', 'Không thể tải đề thi. Vui lòng thử lại.', () => navigate('/dashboard'));
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleOptionToggle = (qId, oId) => {
        ...
        if (answer.selectedOptionIds.has(oId)) {
            answer.selectedOptionIds.delete(oId);
        } else {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            showPopup('success', 'Nộp bài thành công', 'Bài thi của bạn đã được ghi nhận hệ thống.');
        } catch (error) {
            console.error("Failed to submit exam", error);
            showPopup('error', 'Lỗi nộp bài', 'Đã xảy ra lỗi hệ thống hoặc phiên đăng nhập hết hạn.');
        }
    };
    ...
    if (loading) return <div className="loading-container"><h2>Loading secure exam environment...</h2></div>;
    
    if (!exam) return <div className="error-container"><h2>Failed to load exam data!</h2><button className="btn-primary" onClick={() => navigate('/dashboard')}>Go Back</button></div>;
        ...
        <>
            ...
            {(() => {
                ...
                {!exam.showResultToStudent && (
                    <div className="hidden-details-message">
                        <p>Detailed verification has been disabled by the instructor. Your score has been recorded.</p>
                    ...
                    <div className="detailed-review">
                        ...
                        <div className="review-questions-list">
                            {exam.questions.map((q, index) => {
                                const studentAns = attempt.answers && attempt.answers.find(a => a.questionId === q.id);
                                const selectedSet = new Set(studentAns ? studentAns.selectedOptionIds : []);
                                
                                return (
                                    <div key={q.id} className="review-question-item">
                                        <h4>Question {index + 1}: {q.content}</h4>
                                        <div className="options-list">
                                            {q.options.map(o => {
                                                ...
                                                if (isSelected && isCorrect) {
                                                    ...
                                                    optionClass += " correct-missed";
                                                    icon = "✗";
                                                    iconClass += " danger-icon"; // Explicitly makes X red
                                                } else {
                                                    optionClass += " irrelevant";
                                                }

                                                return (
                                                    <div key={o.id} className={optionClass}>
                                                        <input type={q.type === 'SINGLE_CHOICE' ? 'radio' : 'checkbox'} disabled checked={isSelected} />
                                                        <span>{o.content}</span>
                        ...
                        <button className="btn-primary" style={{width: '100%', marginTop: '2rem'}} onClick={() => navigate('/dashboard')}>Return to Dashboard</button>
                    </div>
                )}
            </div>
                    );
            ...
            <div className="exam-main-panel">
                ...
                <div className="questions-list">
                    {exam.questions.map((q, index) => (
                        <div key={q.id} id={`question-${index}`} className="question-item">
                            <h4>
                            ...
                            <div className="options-list">
                                {q.options.map(o => {
                                    const isSelected = answers.find(a => a.questionId === q.id).selectedOptionIds.has(o.id);
                                    return (
                                        <label key={o.id} className={`option-item ${isSelected ? 'selected' : ''}`}>
                                            <input 
                                                type={q.type === 'SINGLE_CHOICE' ? 'radio' : 'checkbox'} 
                                                name={`q-${q.id}`}
                                                checked={isSelected}
                                                onChange={() => handleOptionToggle(q.id, o.id)} 
                                            />
                                            {o.content}
                                        </label>
                                    );
            ...
            <aside className="exam-side-panel">
                ...
                <div className="question-nav-grid">
                    ...
                    <div className="nav-grid-container">
                        {exam.questions.map((q, index) => {
                            const isAnswered = answers.find(a => a.questionId === q.id).selectedOptionIds.size > 0;
                            return (
                                <button 
                                    key={q.id}
                ...
                <button 
                    onClick={() => { 
                        showPopup('info', 'Xác nhận nộp bài', 'Bạn có chắc chắn muốn nộp bài ngay bây giờ?', handleSubmit);
                    }} 
